...
class DeepseekRunner(LLMRunner):
    ...
    def run_one_prompt(self, model_pipeline, row):
        ...
        outputs = model_pipeline(messages, max_new_tokens = 2000, temperature = self.temperature, do_sample=True)
        model_answer = outputs[0]["generated_text"][-1]['content'].split('</think>')[-1]
        return model_answer
